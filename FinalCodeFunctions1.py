...
def TumourSegmentation():
    ...
    pul_seq = 'flair'
    Flair = create_data_onesubject_val(path + 'Brats18_UAB_3446_1_flair.nii.gz')
    pul_seq = 't2'
    T2 = create_data_onesubject_val(path + 'Brats18_UAB_3446_1_t2.nii.gz')
...
def Create3DModel():
    ...
    fl = TumourSegmentation()
    
    image = sitk.GetImageFromArray(fl)
    sitk.WriteImage(image, str(path + 'tumour.mha'))
    image2 = sitk.ReadImage(path + 'Brats18_UAB_3446_1_flair.nii.gz')
...
def TextureEdit():
    with open(str(path + 'modelnew.gltf'), 'r') as myfile:
        data=myfile.read()
    ...
    with open(str(path + 'modelnew.gltf'), 'w') as json_file:
